...
app.get('/api/students/:client', async(req, res) => {
  ...
  const query = 
  `SELECT room_id, student_name, student_username, examiner_username, content, session_id, clientcode 
  FROM messages WHERE session_status = ? AND clientcode = ? AND DATE(created_at) = CURDATE()`;
